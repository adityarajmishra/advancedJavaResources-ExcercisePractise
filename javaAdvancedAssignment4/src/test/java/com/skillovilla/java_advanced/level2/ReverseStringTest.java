...
public class ReverseStringTest {
    ...
    public void testRunMethodOutput() throws Exception {
        ...
        Class<?> reverseStringClass = Class.forName("com.skillovilla.java_advanced.level2.ReverseString");
        Object reverseStringInstance = reverseStringClass.getDeclaredConstructor().newInstance();

        // Get the getRunnable method using reflection and invoke it
        Method getRunnableMethod = reverseStringClass.getDeclaredMethod("getRunnable");
        Object runnableInstance = getRunnableMethod.invoke(reverseStringInstance);

        // Get the run method of the Runnable instance using reflection and invoke it
        Method runMethod = runnableInstance.getClass().getDeclaredMethod("run");
        runMethod.invoke(runnableInstance);
